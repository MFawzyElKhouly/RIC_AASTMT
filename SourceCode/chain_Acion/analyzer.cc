...
//	}
		...
		VecPosition Opp = wm->getOpponent(wm->getOpponentClosestTo(ball));
		VecPosition Intersection_pt = Opp.getVecPositionFromPolar(0.4,Opp.getTheta(),Opp.getPhi());
		scill.setTarget(Intersection_pt);
		scill.setCost(0);
		skillset.push_back(scill);
		return;
	}
