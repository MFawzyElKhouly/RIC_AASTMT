...
double are(VecPosition one, VecPosition two) {
 return one.crossProduct(two).getMagnitude()/2;
}

VecPosition perpPoint(VecPosition opp,VecPosition s, VecPosition d) {
	double ang = (s-opp).getAngleWithVector(d-s);
	if(ang < 90)
		return perpPoint(opp,d,s);
	ang-=90;
	if((s-opp).crossProduct(d-s).getZ() > 0)
		ang*=-1;
	VecPosition p = (s-opp).rotateAboutZ(ang);
	p/=p.getMagnitude();
	double per = perp(opp,s,d);
	if(per ==1000)
		per = 1;
	p*=per;
	p+=opp;
