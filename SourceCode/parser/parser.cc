...
Parser::Parser(WorldModel *worldModel, BodyModel *bodyModel,
		const string &teamName, PFLocalization* particleFilter,
		FrameInfoBlock* vision_frame_info, FrameInfoBlock* frame_info,
		JointBlock* joint_block, SensorBlock* sensor_block) {
	...
	for (size_t i = 0; i < tokens.size() - 1; ++i) {
		...
		if (!(tokens[i].compare("t"))) {
			...
			time_t now = time(0);
			        	...
			        	string mfile;
			sss << "./debugger/" << this->teamName << "_" << ltm->tm_mday
					<< "_" << ltm->tm_hour << "_" << ltm->tm_min << "_"
					<< uNum << ".txt";
			sss >> mfile;
			//cout << "Constructing of Nao Behavior" << endl;
			NaoBehavior::initdebug(mfile);
			++i;
...
bool Parser::parseAccelerometer(const string &str) {
	...
	if (valid) {
		...
		bodyModel->setAccelRates(correctedRateX, correctedRateY,
				correctedRateZ);
...
bool Parser::parseHear(const string &str) {
	...
	double deltaServerToGameTime = worldModel->getTime()
			- worldModel->getGameTime();

	if (processHearMessage(message, hearTime + deltaServerToGameTime, playerNum,
			timeBallLastSeen, ballX, ballY, playerX, playerY, fallen, time)) {
		...
		if (!self && !fallen) {
			...
			WorldObject *pTeammate = worldModel->getWorldObject(
					WO_TEAMMATE1 + playerNum - 1);
...
bool Parser::parseLine(const string &str,
	...
	if (valid) {
		shared_ptr<VisionObject> obj(
				new VisionObject(r, theta, phi, r2, theta2, phi2, lineIndex));
		visionObjs.push_back(obj);
	}
	return valid;
}

bool Parser::parseGoalPost(const string &str,
		vector<boost::shared_ptr<VisionObject> >& visionObjs) {
	...
	if (valid) {
		shared_ptr<VisionObject> obj(
				new VisionObject(r, theta, phi, goalPostIndex));
		visionObjs.push_back(obj);
	}
	return valid;
}

bool Parser::parseFlag(const string &str,
		vector<boost::shared_ptr<VisionObject> >& visionObjs) {
	bool valid = false;
	...
	if (valid) {
		shared_ptr<VisionObject> obj(
				new VisionObject(r, theta, phi, flagIndex));
		visionObjs.push_back(obj);
	}
	return valid;
}

bool Parser::parseBall(const string &str,
		vector<boost::shared_ptr<VisionObject> >& visionObjs) {
	bool valid = false;
	string name;
	double r, theta, phi;
...
bool Parser::parsePlayer(const string &str,
		vector<boost::shared_ptr<VisionObject> >& visionObjs) {
	...
	for (size_t i = 0; i < tokens.size(); ++i) {
		if (!(tokens[i].compare("P"))) {
			//Nothing
		} else if (!(tokens[i].compare("rlowerarm")
				&& tokens[i].compare("llowerarm") && tokens[i].compare("rfoot")
				&& tokens[i].compare("lfoot") && tokens[i].compare("head"))) {
			...
			i += 3;
			if (playerTeamName.compare(teamName)
					|| worldModel->getUNum() != uNum) {
				VecPosition posCartesian =
						(VecPosition(r, theta, phi)).getCartesianFromPolar();
				x += posCartesian.getX();
				y += posCartesian.getY();
				z += posCartesian.getZ();
				notMe = true;
				...
				if (objectIndex != -1) {
					shared_ptr<VisionObject> obj(
							new VisionObject(r, theta, phi, objectIndex));
					visionObjs.push_back(obj);
				}
			}
	...
	if (valid && notMe) {
		...
		if (!playerTeamName.compare(teamName)) {
			...
			shared_ptr<VisionObject> obj(
					new VisionObject(r, theta, phi, playerIndex));
			visionObjs.push_back(obj);
		} else {
...
bool Parser::parseFRP(const string &str) {
	...
	if (valid) {
		if (FRPIndex == FOOT_LEFT) {
			bodyModel->setFRPLeft(VecPosition(centreX, centreY, centreZ),
					VecPosition(forceX, forceY, forceZ));
		} else if (FRPIndex == FOOT_RIGHT) {
			bodyModel->setFRPRight(VecPosition(centreX, centreY, centreZ),
					VecPosition(forceX, forceY, forceZ));
		} else if (FRPIndex == TOE_LEFT) {
			bodyModel->setFRPLeft1(VecPosition(centreX, centreY, centreZ),
					VecPosition(forceX, forceY, forceZ));
		} else if (FRPIndex == TOE_RIGHT) {
			bodyModel->setFRPRight1(VecPosition(centreX, centreY, centreZ),
					VecPosition(forceX, forceY, forceZ));
		} else {
			cerr << "Error: FRPIndex is not valid (this shouldn't happen...)"
					<< endl;
		}
	} else {
		cout << str << " not valid.\n";
...
void Parser::processVision() {
	...
	VecPosition cameraDir = bodyModel->transformCameraToOrigin(
			VecPosition(1, 0, 0));
	SIM::AngDeg cameraAngle = atan2Deg(cameraDir.getY(), cameraDir.getX());
	worldModel->setMyAngDegGroundTruth(
			worldModel->getMyAngDegGroundTruth() - cameraAngle);
	if (worldModel->getMyAngDegGroundTruth() > 180) {
		worldModel->setMyAngDegGroundTruth(
				worldModel->getMyAngDegGroundTruth() - 360);
	...
	for (; begin != end; ++begin) {
		...
		VecPosition objLocalOrigin = bodyModel->transformCameraToOrigin(
				objLocalCamera);
	...
	if (DEBUG_AGENT_ORIENTATIONS && worldModel->getUNum() == 1) {
		...
		worldModel->getRVSender()->drawPoint(pos.getX(), pos.getY(), 10, 0, 1,
				0);
		worldModel->getRVSender()->drawLine(pos.getX(), pos.getY(),
				pos.getX() + dir.getX(), pos.getY() + dir.getY(), 0, .8, 0);
	}
	...
	for (int i = WO_TEAMMATE1; i <= WO_TEAMMATE11; ++i) {
		...
		if (pObj->currentlySeen) {
			...
			VecPosition left;
			VecPosition right;
			WorldObject* bodyObj = worldModel->getWorldObject(
					i - WO_TEAMMATE1 + WO_TEAMMATE_ARM_L1);
			if (bodyObj->currentlySeen) {
				left = bodyObj->pos;
				haveLeft = true;
			}
			bodyObj = worldModel->getWorldObject(
					i - WO_TEAMMATE1 + WO_TEAMMATE_FOOT_L1);
			if (bodyObj->currentlySeen) {
				if (haveLeft) {
			...
			bodyObj = worldModel->getWorldObject(
					i - WO_TEAMMATE1 + WO_TEAMMATE_ARM_R1);
			if (bodyObj->currentlySeen) {
				right = bodyObj->pos;
				haveRight = true;
			}
			bodyObj = worldModel->getWorldObject(
					i - WO_TEAMMATE1 + WO_TEAMMATE_FOOT_R1);
			if (bodyObj->currentlySeen) {
				if (haveRight) {
	...
	for (int i = WO_OPPONENT1; i <= WO_OPPONENT11; ++i) {
		...
		if (pObj->currentlySeen) {
			...
			VecPosition right;
			WorldObject* bodyObj = worldModel->getWorldObject(
					i - WO_OPPONENT1 + WO_OPPONENT_ARM_L1);
			if (bodyObj->currentlySeen) {
				left = bodyObj->pos;
				haveLeft = true;
			}
			bodyObj = worldModel->getWorldObject(
					i - WO_OPPONENT1 + WO_OPPONENT_FOOT_L1);
			if (bodyObj->currentlySeen) {
				if (haveLeft) {
					left = (left + bodyObj->pos) * .5;
				} else {
			...
			bodyObj = worldModel->getWorldObject(
					i - WO_OPPONENT1 + WO_OPPONENT_ARM_R1);
			if (bodyObj->currentlySeen) {
				right = bodyObj->pos;
				haveRight = true;
			}
			bodyObj = worldModel->getWorldObject(
					i - WO_OPPONENT1 + WO_OPPONENT_FOOT_R1);
			if (bodyObj->currentlySeen) {
				if (haveRight) {
					right = (right + bodyObj->pos) * .5;
				} else {
			...
			if (haveLeft && haveRight) {
				...
				pObj->timeOrienLastSeen = worldModel->getTime();
				if (DEBUG_AGENT_ORIENTATIONS && worldModel->getUNum() == 1) {
					cout << "Opponent " << i - WO_OPPONENT1 + 1 << "\t"
							<< pObj->orien << "\n";
					VecPosition pos = pObj->pos;
					VecPosition dir = VecPosition(1, 0, 0);
					dir = dir.rotateAboutZ(-pObj->orien);
					worldModel->getRVSender()->drawPoint(pos.getX(), pos.getY(),
							10, 0, 0, 1);
					worldModel->getRVSender()->drawLine(pos.getX(), pos.getY(),
							pos.getX() + dir.getX(), pos.getY() + dir.getY(), 0,
							0, .8);
				}
			}
		}
	...
	for (int i = WO_BALL; i < NUM_WORLD_OBJS; ++i) {
		...
		if (!pObj->currentlySeen && pObj->validPosition) {
			...
			SIM::AngDeg objAngle = atan2Deg(objPosLocal.getY(),
					objPosLocal.getX()) - bodyModel->getJointAngle(EFF_H1);
	...
	for (int i = 0; i < NUM_AGENTS; ++i) {
		WorldObject* opponent_head = worldModel->getWorldObject(
				WO_OPPONENT_HEAD1 + i);
...
void Parser::processSightings(bool fIgnoreVision) {
	...
	if ((fIgnoreVision || !worldModel->getWorldObject(WO_BALL)->currentlySeen)
			&& worldModel->getWorldObject(WO_BALL)->haveSighting) {
		worldModel->setBall(worldModel->getWorldObject(WO_BALL)->sighting);
		worldModel->setLastBallSightingTime(
				worldModel->getWorldObject(WO_BALL)->sightingTime);
	...
	for (int i = WO_TEAMMATE1; i <= WO_TEAMMATE11; ++i) {
		...
		if (worldModel->getUNum() == i - WO_TEAMMATE1 + 1) {
			if (!worldModel->canTrustVision() && pTeammate->haveSighting) {
				...
				particleFilter->setForTeammateSighting(sighting.getX(),
						sighting.getY(), pTeammate->sightingOrien);
			}
		} else if ((fIgnoreVision || !pTeammate->currentlySeen)
				&& pTeammate->haveSighting) {
	...
	for (int i = WO_OPPONENT1; i <= WO_OPPONENT11; ++i) {
		...
		if ((fIgnoreVision || !pOpponent->currentlySeen)
				&& pOpponent->haveSighting) {
		...
		if ((fIgnoreVision
				|| !worldModel->getWorldObject(
						WO_OPPONENT_HEAD1 + i - WO_OPPONENT1)->currentlySeen)
				&& pOpponent->haveSighting) {
			worldModel->setFallenOpponent(i - WO_OPPONENT1,
					pOpponent->sighting.getZ() < .3);
		}
	}
}

void Parser::computeLocalCornerPositions(VecPosition& fieldXPlusYPlus,
	...
	VecPosition globalPlusPlus, globalPlusMinus, globalMinusPlus,
			globalMinusMinus;
