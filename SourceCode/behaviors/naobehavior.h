...
protected:
    ...
    queue< VecPosition> its;
public:
    static Debugger *debug ;
    double lastswitchingtime = 0;
    static void initdebug(string str);
	...
	VecPosition getHeatMap() {
		static VecPosition cur;
		cur+=worldModel->getBall();
		its.push(worldModel->getBall());
		if(its.size() > 100)
		{
			cur-its.front();
			its.pop();
		}
		VecPosition dav = (cur/its.size()) -avgpos;
		avgpos =cur/its.size();
		return dav;
