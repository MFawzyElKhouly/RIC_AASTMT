 ...
 */
Debugger* NaoBehavior::debug = NULL;
void NaoBehavior::initdebug(string str) {
	NaoBehavior::debug = new Debugger(str);
}
NaoBehavior::NaoBehavior(const std::string teamName, int uNum,
	...
	memory_->getOrAddBlockByName(processed_joint_angles_,
			"processed_joint_angles");
	memory_->getOrAddBlockByName(raw_joint_commands_, "raw_joint_commands");
	memory_->getOrAddBlockByName(processed_joint_commands_,
	...
	parser = new Parser(worldModel, bodyModel, teamName, particleFilter,
			vision_frame_info_, frame_info_, raw_joint_angles_, raw_sensors_);
	...
	resetSkills();
	resetKickState();
	loader = new formationLoader();

	analyzer = new Analyzer(this->worldModel, loader);
...
//    cout << "****************************************\n";
	...
	frame_info_->seconds_since_start = frame_info_->seconds_since_start
			- frame_info_->start_time;

	raw_joint_angles_->values_[RHipYawPitch] =
			raw_joint_angles_->values_[LHipYawPitch];
	...
	if (worldModel->getScoreLeft() != -1 && worldModel->getScoreRight() != -1) {
		scoreMe =
				worldModel->getSide() == SIDE_LEFT ?
						worldModel->getScoreLeft() :
						worldModel->getScoreRight();
		scoreOpp =
				worldModel->getSide() == SIDE_LEFT ?
						worldModel->getScoreRight() :
						worldModel->getScoreLeft();
	...
	if ((worldModel->getPlayMode() == PM_GOAL_LEFT
			|| worldModel->getPlayMode() == PM_GOAL_RIGHT
			|| worldModel->getPlayMode() == PM_BEFORE_KICK_OFF)
			&& worldModel->getLastPlayMode() != worldModel->getPlayMode()) {
	...
	TeamState last = loader->getTeamState();
...
void NaoBehavior::act() {
	...
	if (worldModel->getTime() - worldModel->getLastLineSightingTime()
			> LAST_LINE_SIGHTING_THRESH) {
	...
	bool resetForKickoff = pm == PM_BEFORE_KICK_OFF || pm == PM_GOAL_LEFT
			|| pm == PM_GOAL_RIGHT;
	...
	if (checkingFall()) {
		...
		if (beamablePlayMode()
				&& (worldModel->isFallen() || worldModel->getTime() <= beamTime)) {
			initBeamed = false;
		}
		resetSkills();
		skill = SKILL_STAND;
		...
		if (skills[skill]->done(bodyModel, worldModel)
			...
			case SKILL_WALK_OMNI:
				core->move(velocity.paramSet, velocity.x, velocity.y,
						velocity.rot);
				break;
			case SKILL_STAND:
				core->move(0, 0, 0);
	...
	} else if (ballDistance < 1.0
			&& worldModel->getWorldObject(WO_BALL)->validPosition) {
...
//  cerr << "Loading skills from file " << filename << endl;
	...
	for (int i = 0; i < numRead; ++i) {
		...
		if (c == '$') {
			...
			if (it == namedParams.end()) {
				throw "Missing parameter in skill file " + filename + ": "
						+ param;
			}
	...
	parse_info<iterator_t> info = parse(skillDescription.c_str(), parser,
			(space_p | comment_p("#")));
...
double NaoBehavior::trim(const double& value, const double& min,
		const double&max) {
	double ret;
	if (value > max)
		ret = max;
...
void NaoBehavior::preProcessJoints() {
	for (int i = 0; i < NUM_JOINTS; i++) {
		processed_joint_angles_->values_[i] = spark_joint_signs[i]
				* raw_joint_angles_->values_[i];
	}
}

void NaoBehavior::postProcessJoints() {
	raw_joint_commands_->angle_time_ = processed_joint_commands_->angle_time_;
	raw_joint_commands_->stiffness_time_ =
			processed_joint_commands_->stiffness_time_;
	for (int i = 0; i < NUM_JOINTS; i++) {
		raw_joint_commands_->angles_[i] = spark_joint_signs[i]
				* processed_joint_commands_->angles_[i]; // apply joint signs to convert to the robot's reference frame
		raw_joint_commands_->stiffness_[i] =
				processed_joint_commands_->stiffness_[i];
	}
	raw_joint_commands_->send_stiffness_ =
			processed_joint_commands_->send_stiffness_;
...
VecPosition NaoBehavior::collisionAvoidance(bool avoidTeammate,
		bool avoidOpponent, bool avoidBall, double PROXIMITY_THRESH,
		double COLLISION_THRESH, VecPosition target, bool fKeepDistance) {
	// Obstacle avoidance
	...
	if (avoidBall) {
		if (abs(me.getAngleBetweenPoints(target, ball)) < 90.0
				&& (fKeepDistance
						|| me.getDistanceTo(ball) <= me.getDistanceTo(target))) {
			closestObjPos = ball;
	...
	if (closestObjDistance <= PROXIMITY_THRESH) {
		...
		target = collisionAvoidanceCorrection(me, PROXIMITY_THRESH,
				COLLISION_THRESH, target, closestObjPos);
	}

	return target;
}

VecPosition NaoBehavior::collisionAvoidanceCorrection(VecPosition start,
		double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target,
		VecPosition obstacle) {
	...
	VecPosition left90 = start
			+ VecPosition(0, 0, 1).crossProduct(obstacleDir) * 1.0;
	VecPosition right90 = start
			- VecPosition(0, 0, 1).crossProduct(obstacleDir) * 1.0;
...
VecPosition NaoBehavior::collisionAvoidanceApproach(double PROXIMITY_THRESH,
		double COLLISION_THRESH, VecPosition target, VecPosition obstacle) {
	return collisionAvoidanceApproach(me, PROXIMITY_THRESH, COLLISION_THRESH,
			target, obstacle);
}

VecPosition NaoBehavior::collisionAvoidanceApproach(VecPosition start,
		double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target,
		VecPosition obstacle) {
	...
	if (distanceToObstacle <= PROXIMITY_THRESH) {
		return collisionAvoidanceCorrection(start, PROXIMITY_THRESH,
				COLLISION_THRESH, target, obstacle);
	}
	...
	double pathDistanceFromObstacle =
			(obstacle - closestPathPoint).getMagnitude();
	VecPosition originalTarget = target;
	if (pathDistanceFromObstacle < PROXIMITY_THRESH) {
		target = obstacle
				+ (closestPathPoint - obstacle).normalize() * PROXIMITY_THRESH;
	}
	return target;

}

SkillType NaoBehavior::getWalk(const double& direction, const double& rotation,
		double speed, bool fAllowOver180Turn) {
	return getWalk(WalkRequestBlock::PARAMS_DEFAULT, direction, rotation, speed,
			fAllowOver180Turn);
}

SkillType NaoBehavior::getWalk(WalkRequestBlock::ParamSet paramSet,
	...
	static WalkRequestBlock::ParamSet lastWalkParamSet =
	...
	if (paramSet == WalkRequestBlock::PARAMS_APPROACH_BALL) {
		...
		if (lastWalkParamSet != WalkRequestBlock::PARAMS_APPROACH_BALL
				&& (speed < .5 || abs(direction) > 45)) {
	...
	velocity = WalkVelocity(paramSet, relSpeed * relSpeedX,
			relSpeed * relSpeedY, relRot);
...
SkillType NaoBehavior::goToTargetRelative(const VecPosition& targetLoc,
		const double& targetRot, const double speed, bool fAllowOver180Turn,
	...
	return getWalk(paramSet, walkDirection, walkRotation, walkSpeed,
			fAllowOver180Turn);
}
...
void NaoBehavior::refresh() {
	myXDirection = worldModel->l2g(VecPosition(1.0, 0, 0))
			- worldModel->l2g(VecPosition(0, 0, 0));
	myXDirection.setZ(0);
	myXDirection.normalize();

	myYDirection = worldModel->l2g(VecPosition(0, 1.0, 0))
			- worldModel->l2g(VecPosition(0, 0, 0));
	myYDirection.setZ(0);
	myYDirection.normalize();

	//Anomalous
	myZDirection = worldModel->l2g(VecPosition(0, 0, 1.0))
			- worldModel->l2g(VecPosition(0, 0, 0));
...
void NaoBehavior::getTargetDistanceAndAngle(const VecPosition &target,
	...
	angle = VecPosition(0, 0, 0).getAngleBetweenPoints(myXDirection,
			targetDirection);
	if (isnan(angle)) {
		//cout << "BAD angle!\n";
		angle = 0;
